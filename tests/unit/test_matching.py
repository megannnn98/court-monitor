...
from court_monitor.services import ReprocessWouldDiscardDecisions, import_rfm_records, reprocess
from court_monitor.sources.fedsfm import parse_file
from court_monitor.storage import repository as repo
from court_monitor.storage.orm import (  # noqa: E501
    Base,
    ExtractedFact,
    MatchCandidate,
    PersonRecord,
...
# ---------------------------------------------------------------------------
# Reprocess must not silently discard operator decisions
# ---------------------------------------------------------------------------


def _doc_with_decided_candidate(session, status: str):
    doc = SourceDocument(
        url="https://example.invalid/x",
        source_type="telegram",
        content_hash="rh1",
        parser_status="parsed",
        text="Иванов Иван Иванович осуждён.",
    )
    session.add(doc)
    session.flush()
    fact = ExtractedFact(
...
def test_reprocess_refuses_to_discard_a_confirmed_decision(db_session):
    """Deleting facts cascades to their candidates; a confirmed one is a human
    judgement about a named person and must not vanish silently."""
    doc_id = _doc_with_decided_candidate(db_session, "confirmed")

    with pytest.raises(ReprocessWouldDiscardDecisions) as exc:
        reprocess(db_session, doc_id)
    assert exc.value.decided == 1


def test_reprocess_refuses_for_a_rejected_decision_too(db_session):
    doc_id = _doc_with_decided_candidate(db_session, "rejected")
    with pytest.raises(ReprocessWouldDiscardDecisions):
        reprocess(db_session, doc_id)


def test_reprocess_proceeds_when_only_pending_candidates_exist(db_session):
    """Pending candidates are regenerated by the next match run, so the
    ordinary case must not be blocked."""
    doc_id = _doc_with_decided_candidate(db_session, "pending")
    assert reprocess(db_session, doc_id) == doc_id


def test_force_allows_discarding_decisions(db_session):
    doc_id = _doc_with_decided_candidate(db_session, "confirmed")
    assert reprocess(db_session, doc_id, force=True) == doc_id
