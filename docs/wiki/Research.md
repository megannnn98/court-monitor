...
- **Один запрос — один снимок базы.** `ResearchService.execute()` читает через единицу работы (`research/unit_of_work.py`): `find_person_ids()`, `get_person_details()`, `get_latest_classifications()`, `get_rosfinmonitoring()` и `CandidateQueryService` идут через одну сессию в read-only транзакции `REPEATABLE READ`, которая откатывается в конце (commit не бывает). Переклассификация или повторный matching, закоммиченные посреди запроса, в ответ не попадают. API, CLI, LangGraph workflow, MCP и оценки строят сервис одинаково (`SqlAlchemyResearchUnitOfWork`). Граница: LLM intake и semantic retrieval в Qdrant идут до транзакции (они дают запрос и candidate ids), отчёт строится после неё; «последний snapshot» workflow выбирает до транзакции, а его существование проверяется внутри.
- Поиск по имени (`ILIKE`) по кириллице зависит от ctype/collation конкретной БД.
- Research не разрешает личности: поиск «Иванова» — это фильтр по имени, а не решение «кто один человек». Entity resolution выполняется на границе extraction ([Entity-Resolution](Entity-Resolution.md)); упоминания в статусе pending review (ER v2 REVIEW) не связаны с Person и в результаты research не попадают до решения ревьюера.

## Monitoring findings

Автоматический monitoring ([Monitoring](Monitoring.md), ADR 0013) не пишет собственный SQL для «новых кандидатов»: после каждого run `MonitoringFindingService` вызывает тот же `CandidateQueryService` (POLITICAL + RF `not_matched` для latest imported snapshot) и сохраняет результат в `monitoring_findings` с дедупликацией по `(finding_type, person_id, criteria_version)`, `first_seen_run_id` и `active`. Finding — это событие monitoring workflow, а не статус Person; `ResearchService` и `POST /research` findings не читают и не меняют. Без RF snapshot findings не оцениваются (`no_rf_snapshot`), `ambiguous`/`needs_review` отсутствием не считаются.
