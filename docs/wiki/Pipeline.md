...
| 5 | «Отобрать политические дела и сверить с РФМ», `find-political` | сверяет людей со свежим перечнем, отделяет политическое преследование от обычной уголовщины и ищет безымянных фигурантов | `entity_group_rf_matches`, снимок перечня, `entity_group_politics`, `unnamed_figurants` |
