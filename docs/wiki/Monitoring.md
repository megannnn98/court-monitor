...
`dagster-db-init` создаёт отдельную БД `DAGSTER_PG_DB` (по умолчанию `court_monitor_dagster`); таблицы Dagster не смешиваются с доменными. Semantic-этап в контейнерах: собрать образ с `--build-arg INSTALL_SEMANTIC=1`, поднять `--profile semantic` и задать `MONITORING_QDRANT_URL=http://qdrant:6333`.
