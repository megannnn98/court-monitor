...
| Команда | Флаги |
|---|---|
| `fetch-source <name>` | `--live`, `--limit <n>`, `--no-parse`, `--file <path>`, `--dry-run`, `--replace`, `--force` |
| `fetch-all` | — |
...
```bash
fetch-source fedsfm --live --replace    # заменить перечень целиком (рекомендуется)
fetch-source fedsfm --live              # дозаписать поверх имеющегося
fetch-source fedsfm --file <path>       # из локального XML / DBF / ZIP / CSV
fetch-source fedsfm --file <path> --dry-run   # предпросмотр без записи
```

`--replace` удаляет хранимый перечень перед импортом. Это правильная операция:
источник публикует список целиком, а не приращение. Дозапись корректна только
при совпадении полей у обоих списков, а у CSV-выгрузки нет дат рождения вообще
— тот же человек получает другой ключ дедупликации и задваивается.

Удаление записей каскадом уносит их кандидатов, поэтому `--replace` откажется
работать, если среди них есть рассмотренные оператором; `--force` подавляет.

Ключ дедупликации: `(source, normalized_name, birth_date)`, а при отсутствии
даты — ещё и `birth_place`. Без места рождения ключ вырождается в имя, и на
реальных данных 94 разных человека с распространённым ФИО схлопывались в одну
запись.
...
| `judge-matches` | `--limit <n>`, `--status <str>`, `--rejudge`, `--verbose` |

`judge-matches` спрашивает языковую модель, согласуется ли лицо из документа
с записью реестра, и сохраняет вердикт с обязательной цитатой. Требует
настроенных `CM_LLM_*` (см. `configuration.md`); без них команда выходит с
кодом 1 и говорит, чего не хватает. По умолчанию берёт только неразобранных
кандидатов — `--rejudge` пересматривает и остальных. Расход токенов виден с
`--verbose`.

Автоматического подтверждения нет и не планируется: `generate-matches` только
создаёт кандидатов, а `judge-matches` — только подсказывает; ни то, ни другое
не меняет статус. Решение всегда за человеком. Каждый confirm/reject пишется
