...
Состояние на сегодня — 338 тестов, ruff + mypy чистые.
...
| Structural review, 7 находок | `99dc0e8`…`HEAD` | `SourceStats` владеет счётчиками (чинит потерянный `blocked`), общий цикл fetch, таблица проходов в статьях, разбитый scorer, doctor возвращает проблемы, CLI 1625 → 559 строк |
