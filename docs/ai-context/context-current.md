...
Состояние на сегодня — 339 тестов, ruff + mypy чистые.
...
| README как пошаговый сценарий | `HEAD` | Справочник вынесен в `docs/cli-reference.md` и `docs/configuration.md`; попутно найден и починен `--dry-run` без негативной формы — `import-source-registry` не мог записать реестр |
