...
Etap 5: операторский веб-интерфейс (`web/`) — обзор, документы, очередь совпадений с confirm/reject, очередь проверки и фоновые операции. Подробности в `architecture.md`.

Состояние на сегодня — 310 тестов, ruff + mypy чистые.
...
| Фоновые операции из веба | этот | Таблица `jobs`, один воркер, закрывает D-008 для ad-hoc запуска |
