...
- **`run-all [--live]`** — одна команда: sudrf-источники (`config/sources.yaml`) + все Telegram-каналы из реестра (`config/source_registry.yaml`, раньше не было общего fetch для них) + `generate-matches`. Fixtures по умолчанию, `--live` — реальная сеть. Тонкая оркестрация уже протестированных `process_source`/`process_registry_source`/`generate_matches` — как и у существующего `fetch-all`, отдельных CLI-тестов на саму команду нет, только на pure-helper `_accumulate_fetch_stats`.
- 210 тестов проходят (ruff + mypy чистые).
