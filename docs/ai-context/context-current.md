...
Состояние на сегодня — 316 тестов, ruff + mypy чистые.
...
| Реестр, аудит, `list-audit-log` | этот | Поиск по перечню и просмотр решений в вебе и CLI |
...
2. Если БСР не откроется — `vsrf.ru` доступен, но рендерится через JS. Понадобится `playwright` (в `optional-dependencies`, не установлен; ленивый паттерн подключения уже есть в `sources/airtable_registry.py::render_shared_view`).
